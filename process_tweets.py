...
def extract_fetched_tweets(raw_twitter_input):
    ...
    for tweet in json_data:
        tweets.append(
            Tweet(tweet['text'].encode('utf-8'), tweet['user']['location'].encode('utf-8')))
        processed_tweets += process_raw_tweet(
...
def analyse_tweets(tweets, processed_tweets):
    ...
    proc = subprocess.Popen(["java -cp './stanford-corenlp/*' -mx5g edu.stanford.nlp.sentiment.SentimentPipeline -stdin"],
                            bufsize=100, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    stdout_data = proc.communicate(
        input=processed_tweets)[0].strip().split('\n')
    count = 0
